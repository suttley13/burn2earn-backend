...
/**
 * @param {string|null} base64Image  Raw base64-encoded image data, or null for text-only
 * @param {string} mimeType          e.g. "image/jpeg"
 * @param {string} [userText]        User-provided food description (required when no image)
...
export async function analyzeFood(base64Image, mimeType, userText) {
  ...
  const promptParts = base64Image
    ? [
        { text: userText ? `${PROMPT}\n\nUser note: ${userText}` : PROMPT },
        { inlineData: { mimeType, data: base64Image } },
      ]
    : [{ text: `${PROMPT}\n\nFood description: ${userText}` }]
